    ...
    <div className="p-6 bg-[#FFF8F0] min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-4xl font-bold text-[#5D4037] flex items-center gap-3">
        ...
        {filteredQueries.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredQueries.map((query) => (
              <div
                key={query._id}
                className="bg-white rounded-xl border-2 border-[#E8D5C4] p-6 hover:shadow-lg transition-shadow cursor-pointer"
                onClick={() => setSelectedQuery(query)}
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-[#5D4037] mb-1">{query.name}</h3>
                ...
                <div className="bg-[#FFF8F0] rounded-lg p-3 mb-3 border border-[#E8D5C4]">
                  <p className="text-sm text-[#5D4037] font-semibold mb-1">Query:</p>
                  <p className="text-[#8B7355] line-clamp-2">{query.query}</p>
                </div>

                <div className="flex justify-between items-center text-xs text-[#8B7355]">
                  <span>Order ID: {query.order._id.substring(0, 8)}... | ₹{query.order.total}</span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-8 border-2 border-[#E8D5C4] my-8">
            ...
            <div className="bg-[#FFF8F0] rounded-lg p-4 mb-6 border border-[#E8D5C4]">
              <h3 className="text-lg font-bold text-[#5D4037] mb-3">Customer Information</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-[#8B7355] font-semibold">Name</p>
                  <p className="text-[#5D4037] font-semibold">{selectedQuery.name}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-[#5D4037] break-all">{selectedQuery.email}</p>
                </div>
                <div>
                  <p className="text-xs text-[#8B7355] font-semibold">Phone</p>
                  <p className="text-[#5D4037] font-semibold">{selectedQuery.phone}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4 mb-6 border border-blue-200">
              <h3 className="text-lg font-bold text-blue-900 mb-2">Customer Query</h3>
            ...
            {selectedQuery.adminReply && (
              <div className="bg-green-50 rounded-lg p-4 mb-6 border border-green-200">
                <h3 className="text-lg font-bold text-green-900 mb-2">Our Reply</h3>
            ...
            {selectedQuery.status !== "Resolved" && (
              <div className="space-y-4 border-t border-[#E8D5C4] pt-6">
                <h3 className="text-lg font-bold text-[#5D4037]">Send Reply</h3>
                <textarea
                  value={replyText}
                  onChange={(e) => setReplyText(e.target.value)}
                  placeholder="Type your response here..."
                  rows={4}
                  className="w-full px-4 py-2 border-2 border-[#E8D5C4] rounded-lg text-[#5D4037] placeholder-[#8B7355] focus:outline-none focus:ring-2 focus:ring-[#8B4513] resize-none"
                />
